...
```{r}
# Vectors ----
vector1 <- c(1,2,5.3,6,-2,4) # numeric vector #get 2nd item, vector1[2]
vector1
...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #values 1-20 with 5 rows, 4 columns 
matrix1
...
matrix1[ , 3] #
matrix1[c(12, 14)] #
matrix1[c(12:14)] #getting a slice of data
matrix1[2:4, 1:3] #
...
list1[[2]] #double brackets will assess value of item
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #colon runs 2 commands on one line 

dataframe1[1:2,] #shows rows 1 and 2, all columns 
...
> Answer: Different dimensioned objects appear in different sections in the Environment
...
High.Celsius <- Celsius(High.Temp)
Low.Celsius <- Celsius(Low.Temp)
...
Temp.Data <- cbind(High.Temp, Low.Temp, High.Celsius, Low.Celsius)
Temp.Data
#temps_df <- dataframe()
#names(temps_df) <- c()
```
