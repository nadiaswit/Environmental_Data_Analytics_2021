...
`==` use to compare tpo a specific value
`!=` to check if it is the same
`<`
`<=`
`>`
`>=`
`&` aggregate multiple conditions
`|` slash is used if you want one or the other to be true
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #create subset dataset using matrix filtering
...
# Choose multiple conditions to filter - chooses only rows
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #using "or" operator
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" &  #checking when they are not (excluding symbol)
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #select based on mult elements w/ vector

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) #combination of both conditions, use &
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #comma is combination of both
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #can use column symbol :
...
# find out the start and end dates of the dataset, creates interval object with start/end date
...
NTL.phys.data.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #condition statements after data frame is first
