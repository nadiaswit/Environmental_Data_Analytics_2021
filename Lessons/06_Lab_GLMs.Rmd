...
Similar to an ANOVA but simpler for (have only one or two groupings of data).
...
shapiro.test((EPAair.subsample$Ozone)) #can only run on 5000 records
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) #semi colon runs graphs on top of each other
...
Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  ...
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #fills graph so no space between x axis
print(Ozone.plot)
```
...
> On average, ozone measured in North Carolina is less than 50 ppm(t-test,df=6829,p<0.01)
...
#is ozone a driver and impacts paramaters?
#H0: there is no relationship; Ha: changes in PM2.5 related to changes in ozone
...
#correlation doesn't cause causation. Are there other better variables? Combining factors?
