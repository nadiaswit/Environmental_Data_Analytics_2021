...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #separate tidyverse command - supplies field and what separates by; if use NA instead it will not inlude the NULL columns (3 commands: column be separating,column to be made,what is being separated by)
...
#can also call per column specifically
...
#run starting points for ts and frequency (time step); important why not to exclude NAs bc can mess with time step

Temperature_ts <- ts(BRA_complete$Temperature_C,
                      start=c(f_year,f_month),
                      frequency=12)
#want daily data - use frequency 365
...
Temp_decomp <- stl(Temperature_ts,s.window = "periodic")
plot(Temp_decomp)
#s.window = periodic defines period in data; if replaced with 12 will look the same (because of season); allows to override default and set different time window
#bars at end are scale bars
#first panel = data; seasonal = seasonal fluctuations; trend=stats compute trend; remainder = other data that can be explained by random fluctuations(white noise)
...
Rain.Trend <- SeasonalMannKendall(Rainfall_ts)
Rain.Trend.2 <- smk.test(Rainfall_ts)

Rain.Trend
Rain.Trend.2

Temp.Trend <- smk.test(Temperature_ts)
Temp.Trend
