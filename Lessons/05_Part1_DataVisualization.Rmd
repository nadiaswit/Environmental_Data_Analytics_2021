...
#when applying the dimensions in the chunk title, it will apply to all graphs in chunk
# Scatterplot
...
print(O3plot) #useful when you want to create several plots and want them placed together, when calling object

# Fix this code
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) #when add in aesthetics blue, actually adding another layer
  geom_point(aes(x = Date, y = Ozone), color = "blue") #need to change color outside of aesthetics command
print(O3plot2)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #create one plot for each site
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #y values would be plotted from min to max (want zero to be on top for limnological data to show depth)
print(tempvsdepth)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Color regards contour of shape, fill is for actual box
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) #binaxis = stacking over y, stackdir = stacking points based on center, position = dodge go to size (don't want overlayed dots) to see how many time concentration equals value; no box plot presenting nutrients
print(Nutrientplot4)

# Violin plot - rotated density plot
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
print(Nutrientplot6) #5 different plots based on nutrient; used fill to change color below the curve (color would have been the lines)
