...
# Relative pathway, including one period will go into a folder from the main respository, but 2 periods will look outside the main folder
...
# Rename columns, c short for cbind
...
str(USGS.flow.data) #shows df and all the objects
dim(USGS.flow.data) #how many rows and columns, short for dimension
length(USGS.flow.data)

head(USGS.flow.data) #shows first 6 rows
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5) #show bottom of dataset
USGS.flow.data[30000:30005, c(3, 8, 14)] #row:row and c is for what columns
...
```{r removeNA}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #remove NA to new object
dim(USGS.flow.data)
...
%d  day as number (0-31) #the % is usually used to denote a variable
...
#to view the first few entries, USGS.flow.data$datetime[1:10]; teach R to transform character into date entry
```
...
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d) #last observation, add 20; before, add 19 (concatenating)
       }) #d is the input (whatever assigned to daytime column)
#
