...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #within function(data set, column range, directing info to 2 new columns)

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #subset similar to filter
...
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset) #combine all rows and all columns, fills in NA value if missing
...
Bind elements by row (similar to cbind).
...
LitterFiles #lists all files to be read, set relative pathway

Litter <- LitterFiles %>% #read each element
  ldply(read.csv)
```
...
LitterTrap <- LitterTrap %>%
  select(plotID:trapID, collectDate, functionalGroup:qaDryMass, subplotID:geodeticDatum) #select chooses columns, not rows
