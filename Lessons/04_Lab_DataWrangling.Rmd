...
# dplyr filtering
#NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) #applies mask, doesn't use tidyverse

#matrix filtering
sample1 <- NTL.phys.data[NTL.phys.data$depth]

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #%in% supplies list
...
NTL.year <- filter(NTL.phys.data, year4 == 1999)
class()

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.range <- filter(NTL.phys.data, lakename == 'Tuesday Lake', year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Not as reproduceable. Row numbers constantly change.
...
```{r}
#pipin omits the dataframe argument as being first (will be involved in pipe)
NTL.phys.data.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #filter out 2 lakes
  select(lakename, sampledate:temperature_C) %>% #use colon to select range of rows
  mutate(temperature_F = (temperature_C*9/5) + 32)

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 through 1999 only for July 
Ex3 <- 
  NTL.phys.data %>% #CTL SHFT M
...
Ex4 <-
  Ex3 %>% 
  filter(depth==0) %>% 
  summarize(mean_surface_temp = mean(temperature_C)) #aggregate
...
#reshaping, transforming data; pivot longer stretching rows longer (creating more rows inn dataset). Want to combine 2 columns and then rename (do this for some visualization needs)

Ex5 <- NTL.phys.data %>% 
  pivot_longer(c(irradianceWater, irradianceDeck),
               names_to = "irradiance", #puts names of where the column came from in the name column
               values_to = "rads")
...
Ex6 <- NTL.phys.data %>% 
  pivot_wider(
    ...
    names_from = depth, #get new column names from depth
